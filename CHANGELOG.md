...
### Fixed
- Corrected `ConvertMode.log`/`ConvertMode.ln` mapping (log10 vs natural log) to match API naming.
- Updated documentation samples and usage guidance for NuGet (sample file paths, README/USAGE alignment).
- Restored zero-allocation GridCsvParser/XyzCsvParser (row-major buffers, lazy `Data` materialization, buffer extraction APIs) to reduce parsing allocations.
